...
{
    const int clampedLevel = std::clamp(level, 1, GameConfig::kHeroMaxLevel);
    return GameConfig::kHeroExperienceRequirements.at(clampedLevel);
}
}
hero::hero()
{
    setPosition(100, 100);//起始位置
    Hero_speed = GameConfig::kHeroBaseSpeed;//速度
    resetState();
}
...
int hero::gainExperience(int amount)
{
    ...
    int levelsGained = 0;
    while (m_level < GameConfig::kHeroMaxLevel && m_experience >= experienceRequiredForLevel(m_level + 1)) {
        ++m_level;
        ++levelsGained;
    }

    if (m_level >= GameConfig::kHeroMaxLevel) {
        m_level = GameConfig::kHeroMaxLevel;
        m_experience = experienceRequiredForLevel(GameConfig::kHeroMaxLevel);
    }
...
{
    return GameConfig::kHeroMaxLevel;
}
...
{
    return m_level >= GameConfig::kHeroMaxLevel;
}
...
{
    return QPointF(Hero_x + GameConfig::kHeroShootOriginOffsetX,
                   Hero_y + GameConfig::kHeroShootOriginOffsetY);
}
